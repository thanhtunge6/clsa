...
setup(
    ...
    ext_modules = [
                   Extension("externals.bolt.trainer.sgd",
                             ["externals/bolt/trainer/sgd.c"],
                             include_dirs=[np_inc],
                             extra_link_args=["-O3"],
                             library_dirs=[lib_path,],
                             extra_compile_args=["-O3", "-g"]
                             ),
                   Extension("externals.bolt.trainer.avgperceptron",
                             ["externals/bolt/trainer/avgperceptron.c"],
                             include_dirs=[np_inc],
                             extra_link_args=["-O3"],
                             library_dirs=[lib_path,],
                             extra_compile_args=["-O3", "-g"]
                             ),
                   Extension("externals.bolt.trainer.maxent",
                             ["externals/bolt/trainer/maxent.c"],
                             include_dirs=[np_inc],
                             extra_link_args=["-O3"],
                             library_dirs=[lib_path,],
                             extra_compile_args=["-O3", "-g"]
